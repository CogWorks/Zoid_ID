...
def update_feedback_adaptive(trial_counter, wrong_counter,response_time_counter,pre_post_break_flag):
    ...
    if block_counter + 1 == total_block_num:
        ...
        Feedback_Screen.text = feedback+accuracy_message+speed_message+feedback_message+"You can now take a break. When you are ready to continue, place you fingers on Z and M keys, then press either one.\n(Maximum break time: 5 mins)"
    elif (pre_post_break_flag == "post"):
        Feedback_Screen.text = "Your break has ended.\nGet ready for the next block. Place you fingers on Z and M keys.\n\n" + feedback_message_after_break + "New block starting in 10 seconds..."
    else:
...
def key_to_answer(key,samediff_key_map):
    if (key in samediff_key_map[0]):
        return "same"
    elif (key in samediff_key_map[1]):
        return "diff"
    else:
...
if (DEBUG_MODE):
    blocks = [[((vert_i, 0),(vert_i, 90))],[((vert_i, 0),(vert_i, 90))],[((vert_i, 0),(vert_i, 90))]]
...
samediff_key_map=[["z", "1", "3"], ["m", "2", "4"]]
np.random.shuffle(samediff_key_map)
same=samediff_key_map[0][0]
diff=samediff_key_map[1][0]
...
if samediff_key_map[0]=="z":
    ...
    z_instruction = visual.TextStim(win=win, ori=0, name='z_instruction',
                                    text=z_text,    font=u'Yu Gothic UI',
                                    pos=[-.7, -.2], height=0.1, wrapWidth=None,
                                    color=u'tomato', colorSpace='rgb', opacity=1,
                                    depth=0.0)	
    m_instruction = visual.TextStim(win=win, ori=0, name='m_instruction',
...
for block in blocks:
    ...
    for thisTrial in trials:
        ...
        while continueRoutine:
            ...
            # *Remaining* updates, show every 25 blocks
            if (len(block)-trial_counter) % 25 == 0:
