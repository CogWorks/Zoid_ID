...
def update_feedback_adaptive(trial_counter, wrong_counter,response_time_counter):
	...
	accuracy_message += "You were "+str(round(percent_correct*100,2))+" percent accurate. \n"
	speed_message="Your average response time was "+str(round(average_time,2))+" seconds.\n"
	#determine feedback
	if percent_correct<slow_down:
		feedback_message = "Good job! Try to take more time on each pair to bring your accuracy up!"
	...
	Feedback_Screen.text = feedback+accuracy_message+speed_message+feedback_message+"\nThis screen will end after 10 seconds."
...
total_block = 2
while x < total_block:
	blocks.append(incorrect_matches[int(x*(total_incorrect/total_block)):int((x+1)*(total_incorrect/total_block))])
...
for block in blocks:
	...
	for thisTrial in trials:
		...
		trials.addData('Subject_Response', key_to_answer(Response.keys[-1], yesno)) # what the subject indicated by the key they pressed (same or different)
		trials.addData('Correct_Response', key_to_answer(correct_response, yesno)) #what the correct answer was
	...
	update_feedback_adaptive(trial_counter, wrong_counter, response_time_counter)
