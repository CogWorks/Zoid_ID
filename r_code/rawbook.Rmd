...
sz.summ.sid <- summarySE(data = same.data_sz, measurevar = "Response.rt.box", groupvars = c("Left_Zoid", "deg.diff.level", "SID"))
jl.summ.sid <- summarySE(data = same.data_jl, measurevar = "Response.rt.box", groupvars = c("Left_Zoid", "deg.diff.level", "SID"))
...
jllj.summ <- summarySE(data = all_jl, measurevar = "Response.rt.box", groupvars = c("Type", "deg.diff.level"))
...
tzr_summary <- ddply(raw.data, .(Block_Number, Correct_Response), function (x){
	...
	data.frame(total_blocks, trials_per_block, m_rt, med_rt, correct)
})
...
plt.7a <- ggplot(sz.summ.sid, aes(x = deg.diff.level, y = Response.rt.box, group = Left_Zoid, color = Left_Zoid))
plt.7a + geom_line() + geom_point() + ylim(0, 2) + facet_grid(.~SID)

plt.7b <- ggplot(jl.summ.sid, aes(x = deg.diff.level, y = Response.rt.box, group = Left_Zoid, color = Left_Zoid))
plt.7b + geom_line() + geom_point() + ylim(0, 2) + facet_grid(.~SID) 

plt.8a <- ggplot(sz.summ, aes(x = deg.diff.level, y = Response.rt.box, group = Left_Zoid, color = Left_Zoid))
plt.8a + stat_summary(fun.y = mean, geom = "line") + stat_summary(fun.y = mean, geom = "point") + geom_errorbar(aes(ymin=Response.rt.box-ci, ymax=Response.rt.box+ci), width = 0.05, position=position_dodge(width=0.3)) + ylim(0, 2) + theme_bw()
...
plt.jl + geom_line() + geom_point(size = 2.5) + ylim(0.5, 1.8) + theme_bw() + labs(title = "Response Time with all J L pairs", y = "Response Time", x = "Abs. Degree Difference") + geom_errorbar(aes(ymin=Response.rt.box-ci, ymax=Response.rt.box+ci), width = 0.2) + facet_grid(Type~.)
...
tzr_medRT_by_SameDif_by_blk <- ggplot(tzr_summary, aes(x=Block_Number, y=med_rt, group=Correct_Response, color=Correct_Response)) +
	...
	scale_shape_manual(values=c(17,19)) +
	scale_color_manual(values=c("darkred", "darkblue")) +
	annotate(geom = "text", x=6, y=1.5, label = "Medium Response Time for Correct Response", color = "darkgreen") +
	theme_bw()
