...
jllj.summ <- summarySE(data = all_jl, measurevar = "Response.rt.box", groupvars = c("Type", "deg.diff.level", "Crit.level"))
...
plt.jl + geom_line() + geom_point(size = 2.5) + ylim(0.5, 2.5) + theme_bw() + labs(title = "Response Time with all J L pairs by high/mid/low skill levels (NA for missing scores)", y = "Response Time", x = "Abs. Degree Difference") + geom_errorbar(aes(ymin=Response.rt.box-ci, ymax=Response.rt.box+ci), width = 0.2) + facet_grid(Type~Crit.level)
